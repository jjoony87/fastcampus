...
- 하드코딩된 색상값(hex, rgb, hsl) 사용 금지. 반드시 `var(--color-*)` 사용. (예: `color: var(--color-brand-primary)`)
- 하드코딩된 스페이싱 사용 금지. 반드시 `var(--spacing-*)` 사용.
- 타이포그래피는 `var(--font-*)`, `var(--text-*)` 토큰만 사용. font-size 직접 입력 금지, 텍스트 스타일 클래스 참조.
- [중요] 폰트는 Pretendard 사용.
...
- props는 Figma 프로퍼티 기준으로 정의.
- 에이전트 위임 시: 연속 구현 가능. 단, 5개 이상이면 새 세션 권장.
- 에이전트 없이 직접 구현 시: 한 번에 하나의 컴포넌트만.
- `src/components/ui/`에 기존 컴포넌트가 있으면 재사용 — 중복 생성 금지. (예: `<button>` 태그 임의 생성 금지)
